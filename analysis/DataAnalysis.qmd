---
title: "Community Regen Analysis"
format: html
---

# Set up
Code from Trevor, modified 9/05/2024, original in Reburns_Understory_updated.R
...
```{r}
#| label: load files
understory <- read.csv("Upland_UndCover.csv") 
FGkey <- read.csv("Understory_FG.csv") ## functional group key
...
## Create addendum for species without functional groups
FGkey <- FGkey %>%
  bind_rows(data.frame(Type = c("forb", "decid shrub", "Lichen", "deciduous trees", "Moss", "decid shrub"),
                       acronym = NA,
                       Genus = NA,
                       Species = unique(understory_long$species[is.na(understory_long$FG)]),
                       Notes = NA))

## Add final functional groups to understory_long
understory_long <- understory_long %>%
  mutate(FG = FGkey$Type[match(species, FGkey$Species)])
...
understory$simpson <- vegan::diversity(understory_wide, index = "simpson") ## calculating simpson diversity index
...
par(mfrow = c(1,2))
set.seed(1)
NMDSord <- vegan::metaMDS(understory_wide, try = 1000, distance = "bray") ## convergence
...
```{r}
#| label: Bray-Curtis Dissimilarity analysis
